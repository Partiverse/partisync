...
//!
//! 流程：
//! - 发起方：随机 X25519 → 12 词（128 bit 截位编码）→ 入库 ephemeral_sk/pk +
//!   code → 监听 iroh 发现端口；
...
fn mnemonic_to_entropy(words: &str) -> Result<[u8; 16], PartisyError> {
...
pub async fn initiate_pairing(
    ...
    initiator_dev: &str,
    ttl_ns: Option<i64>,
) -> Result<(String, String, [u8; 32], [u8; 32]), PartisyError> {
...
/// 统一入口：发起方生成可重用 32 字节私钥 + 公钥。
///
/// # Errors
/// 随机源失败 → Fatal。
pub fn unified_secret_from_rng() -> (StaticSecret, PublicKey, [u8; 32]) {
    let mut bytes = [0u8; 32];
    fill(&mut bytes).expect("OS RNG 不可用");
...
pub async fn accept_pairing(
    ...
    responder_store
        .complete_pairing(&session.id, responder_dev, &shared_bytes)
        .await?;
    // 同步发起方会话状态为 closed（生产路径通过 iroh 收到接收方 ACK 后做）
    // 这里直接 SQL 即可（Store API 未暴露专门函数，复用 complete_pairing 幂等）
    initiator_store
        .complete_pairing(&session.id, &session.initiator_dev, &shared_bytes)
        .await?;
    Ok(shared_bytes)
}
...
pub fn derive_initiator_keys(
    sk_bytes: &[u8; 32],
    epk_b_bytes: &[u8; 32],
) -> Result<([u8; 32], [u8; 32]), PartisyError> {
    ...
    let mut device_key_arr = [0u8; 32];
    device_key_arr.copy_from_slice(&device_key);
    let mut sk_b = *sk_bytes;
    sk_b.zeroize();
    Ok((shared_bytes, device_key_arr))
}
...
/// # Errors
/// 无（blake3 输入长度固定）。
pub fn derive_device_key(shared_secret: &[u8; 32]) -> [u8; 32] {
...
pub fn endpoint_secret(shared_secret: &[u8; 32]) -> [u8; 32] {
    let mut h = blake3::Hasher::new();
    h.update(shared_secret);
