/* Sheet: Base UI (@base-ui/react/dialog) */
"use client"
...
function SheetOverlay({ className, ...props }: React.ComponentProps<typeof Dialog.Backdrop>) {
  return (
    <Dialog.Backdrop
      ...
      className={cn(
        "fixed inset-0 z-50 bg-black/10 transition-opacity duration-150 data-[state=closed]:opacity-0 data-[state=open]:opacity-0 data-[state=open]:animate-in data-[state=closed]:animate-out supports-backdrop-filter:backdrop-blur-xs",
        className
      )}
      {...props}
    />
