...
# 失去引用的 tags）+ 资产卷内不再被引用的对象 + Meilisearch 对应文档。
...
PATTERNS="name LIKE 'p2-%' OR name LIKE 'p2.%' OR name LIKE 's8-%' OR name LIKE 's2-%' \
...
# 导出待删 Meili 文档 id 与（被删后）不再被引用的对象路径，随后逐项清点。
"${PSQL[@]}" "SELECT id FROM assets WHERE ($PATTERNS)" > /tmp/cleanup-meili-ids.txt
"${PSQL[@]}" "SELECT path FROM assets WHERE ($PATTERNS)" > /tmp/cleanup-deleted-paths.txt
"${PSQL[@]}" "DELETE FROM assets WHERE ($PATTERNS); SELECT 'assets left: ' || count(*) FROM assets;" | tail -1

say "Meilisearch delete-batch"
IDS_JSON=$(jq -R -s 'split("\n") | map(select(length > 0))' /tmp/cleanup-meili-ids.txt)
...
say "资产卷孤儿对象清扫"
# keep 集合 = 现存 assets.path；卷内其余对象均为孤儿，删除。
...
if [ "${REMOVED:-0}" -gt 0 ]; then
  tr '\n' '\0' < /tmp/cleanup-orphans.txt | \
    docker exec -i partisync-server sh -c 'xargs -0 -n 50 rm -f'
fi
echo "orphan objects removed: $REMOVED"
TMPLEFT=$(docker exec partisync-server sh -c 'find /data/assets/.tmp -type f 2>/dev/null | wc -l')
echo ".tmp leftover: $TMPLEFT"
...
rm -f /tmp/cleanup-meili-ids.txt /tmp/cleanup-deleted-paths.txt /tmp/cleanup-keep-paths.txt
echo "DONE"
