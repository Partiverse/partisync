...
## 口径修正（B-06，P2 收尾 2026-09-13——不改动历史数字，仅修正措辞）

- §2.2 标题"后端多维属性过滤**与排序**"：排序在本文写作时点实测是坏的（T6′ C2-2 sort 必 502、C2-3 静默丢弃，
  已于 T6′ 修复批次关闭并回归）。引用本文时排序能力以 T6′ 修复证据（`l2-integration-t6-prime-fix.md`）为准。
- "已实测索引到 1,000,000 条"与所附 `Only Search=true` 日志的矛盾：以 `benchmark-1m.md` 两轮独立复现为准
  （p95 44.97/45.02ms，客户端饱和口径，见该文件 §口径修正）。
