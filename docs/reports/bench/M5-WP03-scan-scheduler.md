...
## 4.5. 修订记录（M5-WP09, 2026-09-25）

后续卡 4「semaphore 协调」实现评估推翻：`Semaphore::close()` 与 fail-fast
路径 `enqueue` 存在 `add_permits on closed` panic 竞态，工作票/唤醒票
区分复杂度高——**改走空闲动态退避**（worker 空转 sleep 1ms 指数升至
50ms，取到分片重置；正确性不依赖时序）。复跑 t06 基准
（234 分片 × 20ms 延迟模型）：

| workers | 原（固定 50ms tick，60 分片外推） | **动态退避（234 分片实测）** |
|---|---|---|
| 1 | ~5.3s | 5.16s（理论 4.68s） |
| 4 | 2.60× | **3.87×**（1.334s，理论下限 1.17s） |
| 8 | 2.39×（无增益） | **7.34×**（0.703s——8 workers 恢复增益） |
| fs 4w | 24.7k 项/s（60 目录口径） | **73.3k 项/s**（60 目录实测） |

8 workers 无增益的结论**作废**——根因即固定 tick 尾相；后续卡 4 清偿。
