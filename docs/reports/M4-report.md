...
- **本地签字复核（2026-09-23，HEAD `d00c1ad`，`NK_TARGET_*=0` 屏蔽 numkong SIMD——Apple Clang 16
  ...
  **退出码 0，测试门禁绿。**
- **CI 复核（关账后 `aefdee0` 推送触发，run #35817331955）**：fmt 24s ✅ / clippy 4m24s ✅ /
  test(ubuntu) 11m5s ✅ / test(macos) 10m51s ✅ / interop ✅ / deny ✅ —— **6 jobs 全 success**，
  D3 清偿在远端验证通过（clippy workspace 零警告）。后续 `3de5f54`（D2 LLM-only 报告）
  再推亦绿。
- **CI 历史快照（`12f3053`，2026-09-21，关账放行签字时点最近一次）**：fmt ✅ / clippy ✅ /
  test(ubuntu) ✅ / interop ✅；test(macos) ❌（`iroh_keyspace` 2 测试——其后 `47276ca` keyspace
  并行隔离修复（2026-09-22）落地）；deny ❌（cargo-deny-action 容器 musl 工具链未装 +
  当时 licenses 项——其后 ADR-0020 deny 基线进场，本地现四段绿）。
...
- **外部渗透（§7.2 M4 触发点）**：**LLM-only 评审已交付**（SEC-AI-PENTEST-M4-WP06-001，2026-09-23）——
  5 模型 × 32 威胁 = 160 calls，167.8s 跑完，$0 成本；修订后阈值 P0=1（T3.4.I `include_vectors`
  默认 true 导出向量 + 3/3 共识 + CVSS 7.5）/ P1=1（tooling/route 路径穿越）/ P2=17 / P3=13。
  **LLM-only 不替代人类 vendor 审计签字**（执行方案 §7.1.10 仅 OSCP/OSWE 持证人有效）——
  后续合规路径：OSCP 持证人 0.5 天复核本报告 + 内部 18 探针 → 一页签 → 闭合 D2。
  原始报告：`docs/reviews/SEC-AI-PENTEST-M4-WP06-001.md`；威胁输入 fixture：
  `docs/security/wp06_threats.json`（32 条手工转录 + 7 条新增）。
- **跨里程碑债务**：M2-D1 外部密码学审计仍未补做（延续挂账，§5-D1）；**D2 由「vendor 委托挂 M5」修订为
  「LLM-only 已交付 + OSCP 0.5 天复核即满足」（§5-D2 修订）**。
...
| D9 | **CI 完整性**：47 提交（`12f3053..d00c1ad`）未推送、未触发 CI；恢复推送节奏 | **关账后立即** → ✅ **已恢复**（`aefdee0` 推送触发 run #35817331955 全绿；后续 `3de5f54` 再推亦绿） |
...
10. M2/M4 外部审计无未关闭高危 —— ✅ **LLM 多模型对抗已交付**（SEC-AI-PENTEST-M4-WP06-001，
    修订阈值 P0=1 / P1=1 / P2=17 / P3=13，交叉验证一致性 2/3）；**剩余 OSCP 持证人 0.5 天复核
    即满足 §7.1.10 合规签字**（~$5k，挂 M5 开局 P1）；内部 0 高危/0 中危；3 P1 未修已登记
    M4-WP99-T02/03/04；M2-D1 密码学外部审计延续挂账
...
2. M5 开局三立即项：D3 ✅ clippy 已清偿（aefdee0）→ M5 直接跳过；D2 OSCP 0.5 天复核
   （~$5k，已补 LLM-only 报告作预热）；D9 ✅ 推送已恢复（run #35817331955 全绿）；后续 D4 加固；
...
- [x] 安全负责人（M2/M4）：@lead——**附条件放行**：内部渗透 0 高危/0 中危；3 P1 未修登记
      M4-WP99-T02/03/04；**外部渗透 LLM-only 评审已交付**（SEC-AI-PENTEST-M4-WP06-001，
      修订阈值 P0=1 / P1=1），**OSCP 持证人 0.5 天复核即满足 §7.1.10 合规签字**（挂 M5 P1）；
      M2-D1 外部密码学审计延续挂账
- [ ] OSCP 复核签字：⬜ 待 M5 开局委派 OSCP 持证人 0.5 天复核（预留位）

**M4 附条件正式放行。** 条件即 §5 债务登记（D2 修订为「LLM-only + OSCP 短复核」；D3 ✅ 清偿；D9 ✅ 推送恢复）。
