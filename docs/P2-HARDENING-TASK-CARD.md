...
| A2-06 | `ReadTimeout=15s` 导致慢速大文件上传 400 | 安全 low | `ReadHeaderTimeout=15s` 保持防慢速攻击；服务端级超时维持 15s/30s，上传端点用 `http.ResponseController` 按请求放宽读写期限（5min/5min+30s） |
...
- `cmd/server`：服务端级 `ReadTimeout`/`WriteTimeout` 维持 15s/30s；上传端点按请求放宽到 5min/5min+30s（`ResponseController`，A2-06 实际机制——口径修正于 P2.1，原文误写为全局 `ReadTimeout=5min`）。
